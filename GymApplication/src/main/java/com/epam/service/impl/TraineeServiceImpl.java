...
public class TraineeServiceImpl implements TraineeService {
	...
	public List<TrainerDto> updateTraineesTrainers(UpdateTraineesTrainerListRequest updateTraineesTrainerListRequest) {
		...
		updateTraineesTrainerListRequest.getListOfTrainerUsernames().stream().forEach(tr -> {
			Trainer trainer = trainerRepository.findByUserUsername(tr)
					.orElseThrow(() -> new TrainerException("No such user been assigned as trainer"));
			trainersList.add(trainer);
			trainer.getListOfTrainees().add(trainee);
			trainer.setListOfTrainees(trainer.getListOfTrainees());
			trainerRepository.save(trainer);
			trainers.add(TrainerDto.builder().firstname(trainer.getUser().getUsername())
